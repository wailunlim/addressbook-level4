...
Before user can use Heart² application, they must first log in with a registered account.

==== Before logging in
User is presented with a login UI. There are only 2 commands available for user to execute:

* `login` : Login to the system with a username and password
* `help` : Shows the help panel

==== After logging in
User is presented with the main-screen UI. User can execute any commands available, provided the user-account is given the correct privilege. However, user cannot execute the `login` command again since he is already logged in.

=== Account creation and account storage
