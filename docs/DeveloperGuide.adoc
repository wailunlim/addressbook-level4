...
Then, we obtain the list of contacts from the database and convert it to a stream and use the `Predicate` to filter the contacts and display the results.

==== [To do in v2.0] Substring matching

Instead of doing an exact match for keywords, we will search for a substring match in the contacts instead.
// end::list[]
