...
There are only 2 commands available for user to execute:
...
The restrictions of a `READ_ONLY_USER` is enforced by the methods found in `Role` enum, specifically:
...
*_Heart²_* allows you to remove 'clients' and 'service providers' if you so wish to!
...
*_Heart²_* allows you to make changes to existing 'clients' and 'service providers' too!

With the list of 'clients' or 'serviceproviders' shown on the GUI, simply specify the index of the contact in question, followed by the field to be edited:
...
This EditContactDescriptor is then used in conjunction with the earlier in contact in question to create a new contact. A check on whether the contact is a 'client' or 'service provider' is first performed to create a correct replacement contact, before updating the contact list. 
...
Then, we obtain the list of contacts from the `Model` and convert it to a stream and use the `Predicate` to filter the contacts and display the results.
