...
The above commands adds a `client` and a `service provider` together with the details provided respectively.
...
When adding a contact, either a new `Client` or a `Vendor` is instantiated.
Both `Client` and `Vendor` objects are added to a list of generic type `Contact`.

In order to differentiate them, there  is a abstract method `Contact#getType()` that `Client` and `Vendor` implements differently.
